...
- 考试平台是成熟软件门类，题库、试卷、考试发布、在线作答、自动评分、成绩统计、用户权限和管理后台都必须先参考成熟实现，再进入 gaokao 的设计。
- 参考项目目录默认位于 `C:\Users\Administrator\Desktop\exam-platform-research`。后续可追加新的成熟项目，但不得把参考项目源码复制进 gaokao 作为运行时代码。
- 中大型功能进入实现前，必须在 `plan.md` 写明参考了哪些项目、吸收了哪些模式、舍弃了哪些模式，以及这些取舍如何服务 gaokao 的当前目标。
- 优先吸收成熟项目的领域拆分、数据库边界、页面信息架构、权限模型、考试状态流、提交锁定、评分记录和错误处理方式。
- 参考不是照搬。第三方代码、表结构、页面和接口只能作为证据与设计输入；进入 gaokao 的实现必须符合当前技术栈、命名体系、测试标准和产品边界。
