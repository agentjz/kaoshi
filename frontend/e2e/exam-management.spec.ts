...
test.describe('考试管理', () => {
  ...
  test('考试管理支持搜索、新建、保存草稿、发布和详情读回', async ({ page }) => {
    ...
    await examDialog.getByRole('button', { name: '生成题目明细' }).click()
    ...
    await expect(examDialog.locator('.paper-table tbody tr').first()).toContainText('Which words are nouns?')
    ...
    await expect(savedDialog.locator('.paper-table tbody tr').first()).toContainText('Which words are nouns?')
    ...
    await expect(editDialog.locator('.paper-table tbody tr').first()).toContainText('Which words are nouns?')
