...
test.describe('考试管理', () => {
  ...
  test('考试管理支持搜索、新建、组卷、次数、显示方式、权限配置和编辑取消', async ({ page }) => {
    const consoleIssues = collectConsoleIssues(page)
    await login(page)
    ...
    await examDialog.getByLabel('考试名称').fill(`浏览器考试 ${Date.now()}`)
    ...
    await examDialog.getByLabel('开放范围').getByText('完全公开').click()
    ...
    await page.getByRole('button', { name: '搜索' }).click()
    await page.getByRole('row').filter({ hasText: '英语基础模拟考试' }).getByRole('button', { name: '编辑' }).click()
    await expect(page.getByRole('dialog', { name: '编辑考试' })).toBeVisible()
