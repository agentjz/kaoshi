...
test.describe('系统管理', () => {
  ...
  test('部门管理支持在线新建、编码生成、编辑和删除', async ({ page }) => {
    ...
    await expect(page.locator('.detail-pane')).toContainText('浏览器验收部')
    await page.locator('.detail-pane').getByRole('button', { name: '新建下级' }).click()
    const childDialog = page.getByRole('dialog', { name: '新建部门' })
    await childDialog.getByLabel('部门名称').fill('浏览器验收小组')
    await childDialog.getByRole('button', { name: '保存' }).click()
    await expect(page.getByText('部门已创建')).toBeVisible()
    await expect(page.locator('.detail-pane')).toContainText('浏览器验收小组')
    await page.locator('.detail-pane').getByRole('button', { name: '编辑' }).click()
    await expect(page.getByRole('dialog', { name: '编辑部门' })).toBeVisible()
    await page.getByRole('dialog', { name: '编辑部门' }).getByLabel('说明').fill('树形部门读回')
    await page.getByRole('dialog', { name: '编辑部门' }).getByRole('button', { name: '保存' }).click()
    await expect(page.getByText('部门已更新')).toBeVisible()
    await page.locator('.detail-pane').getByRole('button', { name: '删除' }).click()
    await page.getByRole('dialog', { name: '删除部门' }).getByRole('button', { name: '删除' }).click()
    await expect(page.getByText('部门已删除')).toBeVisible()
    await page.locator('.tree-pane').getByText('浏览器验收部').click()
    await page.locator('.detail-pane').getByRole('button', { name: '删除' }).click()
    await page.getByRole('dialog', { name: '删除部门' }).getByRole('button', { name: '删除' }).click()
    await expect(page.getByText('部门已删除')).toBeVisible()
