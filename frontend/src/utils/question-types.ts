...
export function isManualReviewType(type: QuestionTypeCode) {
  return questionTypeMeta(type).manualReview
}

export function isMultipleAnswerType(type: QuestionTypeCode) {
  return questionTypeMeta(type).multiple
}
