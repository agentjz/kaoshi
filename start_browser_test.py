...
def main() -> int:
    ...
    try:
        wait_for_http(FRONTEND_URL, stack)
...
def wait_for_http(url: str, stack: subprocess.Popen[bytes], timeout_seconds: int = 300) -> None:
    print("等待真实测试环境可访问...")
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        if stack.poll() is not None:
            raise RuntimeError(f"真实测试环境启动失败，退出码：{stack.returncode}")
        try:
            ...
            time.sleep(2)
    raise TimeoutError("等待真实测试环境超时。请查看 logs\\e2e 里的 backend/frontend 日志。")
